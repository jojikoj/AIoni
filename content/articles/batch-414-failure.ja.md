...
**結論から**：553件の一括処理のうち**414件（75%）が失敗**しました。原因は「どのAIモデルを使うか」をスクリプトに書き忘れたことだけです。上位モデルが使われて利用枠を食い潰し、社長本人の作業と、動いていた分析処理3本まで巻き添えで止まりました。**外部APIの従量課金は使っていないので請求は増えていません。** 失ったのは処理時間です。再発防止に入れた5つの仕組みは、どれも「気をつける」ではなく「**気をつけなくても事故らない**」形にすることを狙っています。
...
| # | 対策 | 何を防ぐか |
|---|---|---|
| 1 | **モデルを明示**し、環境変数で上書き可能に | 既定がHaikuなので、何も考えずに実行しても安全側に倒れる |
| 2 | **8件連続で失敗したら中断** | 414件の空振りを許した最大の理由が「失敗しても止まらない作り」だった |
| 3 | 失敗に**「処理済み」フラグを付けない** | 次回に再試行できる。付けてしまうと永久に処理されない |
| 4 | **5件ごとに逐次保存** | 途中で落ちても、そこまでの成果が残る |
| 5 | **1回あたりの件数を絞り、事前に疎通確認** | 全件を一度に回して全部失う事態を避ける |
...
- 553件中**414件（75%）が失敗**。原因は「どのAIモデルを使うか」を書き忘れただけ
- 上位モデルが使われて利用枠を食い潰し、**社長本人の作業と分析処理3本まで巻き添え**で止まった
- 従量課金は使っていないので**請求は増えていない**。失ったのは処理時間
- 対策の要点は「気をつける」ではなく「**気をつけなくても事故らない**」形にすること。既定値を安全側（Haiku）にした
- **失敗しても止まらない作り**が、414件の空振りを許した最大の理由。8件連続で失敗したら中断するようにした
- **失敗に「処理済み」フラグを付けない。** 付けると次回に再試行されず、永久に処理されない
- **途中経過を保存する。** これだけで事故は「全損」から「途中まで進んだ」に変わる
- **裏で回す処理が、表の仕事を止めないか確認する。** バッチは単体で完結しない。誰の枠を使い、止まったとき誰が困るかを走らせる前に確かめる

自動化が止まっても気づけなかった記録は[止まったメディア更新を、見張り役が自動で叩き起こしていた話](../toe-media-watchdog/)に、動いているはずのジョブを数えた話は[毎朝動いているはずのAI自動化41本を5.3日ぶん数えたら](../toe-cron-firing-audit/)にあります。

この件を含む当社の失敗12件を、起きた場所ごとに整理したものが[AI導入で当社が実際にやらかした12件](../ai-failure-cases-own/)です。モデルの決め方そのものは[生成AIのモデルはどう選ぶか](../ai-model-selection/)にまとめました。
