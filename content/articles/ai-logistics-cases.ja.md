...
**結論から**：運送・物流のAI活用で公表されている数字は、**その性質が一つずつ違います**。佐川急便の「時間短縮を確認」は実地検証の結果ですが具体的な分数は非公開、ヤマト運輸の「最大20%・最大25%」はリリースに**両社予想値と明記された導入前の見込み**、Amazonの「10%改善」は巨額投資を続けた自社倉庫での成果です。**同じ「%」でも、実測値・予想値・自社限定の成果が混在しています。** 検討時に見るべきは数字の大きさより、その数字がどう出たものかです。

### 公表されている取り組みと、その数字の性質

| 企業 | 取り組み | 公表されている数字 | 数字の性質 |
|---|---|---|---|
| 佐川急便（SGホールディングス） | 配送ルート最適化「Loogia」 | ルート組み・配送業務時間・走行距離の短縮を確認 | **実地検証**（14日間・80件）。ただし具体的な分数・距離はリリース本文に記載なし |
| ヤマト運輸 × アルフレッサ | 配送業務量の予測と適正配車 | 配送生産性 最大20%向上／CO2排出量 最大25%削減 | **両社予想値**とリリースに明記。導入後の実測ではなく導入前の見込み |
| Amazon | 倉庫内ロボットの基盤モデル「DeepFleet」 | ロボット群の移動時間 10%改善 | 自社倉庫ネットワークでの成果。巨額の設備投資が前提 |
...
- 公表されている数字は**性質が一つずつ違う**。実地検証の結果、導入前の予想値、巨額投資を前提とした自社成果が混在している。**大きさより、どう出た数字かを見る**
- 佐川急便の試験導入では、**経験の浅いドライバーほど継続利用を希望**した。AIは誰にでも均等に効く道具ではなく、経験や土地勘という暗黙知を補う場面で効きやすい
- 同社は14日間80件の実地検証 → 全国約500名での試験導入 → 全国展開、と**段階を踏んでいる**。いきなり全社導入していない
- 「導入したが使われない」失敗は実際に起きる。原因は、明文化されていない現場の判断（この荷主は急ぎ、この取引先は融通する）をAIへの指示に書き出せないこと
- 自社で最初にやるのは、**「経験の浅い人ほど時間がかかっている作業」「属人化してベテラン頼みの工程」の洗い出し**
- 検証の軸は数字だけでなく**「現場が使い続けたいと思うか」**

AI導入の費用感は[AI導入費用の相場と内訳](../ai-cost-structure/)に、在庫・需要予測での事例は[在庫管理・需要予測にAIを使う](../ai-inventory-demand/)に整理しています。
