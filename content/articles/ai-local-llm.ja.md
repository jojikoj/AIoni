...
**結論から**：ローカルLLMは「情報を外に出さない」ための選択肢ですが、**導入支援サービスの参考価格は500万円から**で、GPU環境の規模がそのまま動かせるモデルの上限になります。クラウド事業者の最新モデルと同じ品質が常に得られるとは限りません。**「ローカルかクラウドか」を全社一律で決める必要はなく**、機密性の高い一部の業務だけをローカルにし、残りはクラウドを使う併用が現実的です。
...
| 論点 | ローカルLLM | クラウドの生成AI |
|---|---|---|
| **情報の流れ** | 自社の機器内で完結。外部事業者のサーバーを経由しない | 入力内容が一度は外部を経由する |
| **費用** | 導入支援の参考価格は**500万円から**（初期導入費用は別途） | 月額または従量課金 |
| **性能の上限** | **用意できるGPU環境の規模が、動かせるモデルの上限**になる | 大規模な計算資源の上で動く最新モデルを使える |
| **必要な知識** | GPUサーバー・大容量メモリーの調達と運用 | 契約すれば使える |
...
- ローカルLLMの目的は「**情報を外に出さない**」こと。顧客情報・図面・未公表の社内文書を扱う業務で意味がある
- ただし**導入支援サービスの参考価格は500万円から**（初期導入費用は別途）。最短1か月での構築に対応するサービスもある
- **用意できるGPU環境の規模が、動かせるモデルの上限**になる。クラウドの最新モデルと同じ品質が常に得られるとは限らない
- どの程度の差が出るかは業務による。**PoCで自社の業務データを使って確かめる**
- **全社一律で決める必要はない。** 機密性の高い一部だけローカル、残りはクラウド、という併用が現実的
- 閉域で完結させたい処理と、そうでない処理を**分けて設計する**

費用の考え方は[AI導入費用の相場と内訳](../ai-cost-structure/)に、ツールの選び方は[AIツールの選び方](../ai-tool-selection/)に整理しています。
