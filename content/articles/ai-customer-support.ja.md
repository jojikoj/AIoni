...
**結論から**：問い合わせAIで変わるのは「**単純な繰り返し質問への一次対応にかかる時間**」です。クスリのアオキは対応業務負荷を75%削減しました。ただし**その数字は1年間の運用を経た実績**で、開始時点のQ&Aは約300件でした。導入初日から出る効果ではありません。そして**新たに必要になるのが「FAQを作り、更新し続ける運用」**です。この手間を見込まずに入れると、効果が出る前に「使えない」と判断されます。
...
| | 中身 |
|---|---|
| **変わったこと** | 単純な繰り返し質問への**一次対応にかかる時間**。クスリのアオキでは労務課が同じ説明を繰り返す時間が減り、RICOHではヘルプデスクの対応件数が圧縮された |
| **変わらなかった／新たに必要になったこと** | **FAQを作り、更新し続ける運用の手間**。精度は導入時点で完成せず、寄せられた質問と回答のずれを継続的に埋める作業が前提 |
...
- 変わるのは**単純な繰り返し質問への一次対応の時間**。クスリのアオキは対応業務負荷を**75%削減**（従業員約1万5000人）
- **その数字は1年間の運用を経た実績。** 開始時点のQ&Aは約300件で、初日から出た効果ではない
- 精度は**FAQの量と質**に左右される。運用しながら段階的に上げていくもの
- 失敗の共通原因は「**導入目的が明確でないまま導入する**」「**運用改善を怠る**」の2つ
- 新たに必要になるのは**FAQを作り、更新し続ける手間**。これを見込まないと、効果が出る前に「使えない」と判断される
- 導入後しばらくは**回答精度を上げるための学習期間**が要る、という前提で始める
