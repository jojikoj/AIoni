---
title: AI面接・AI採用のバイアス対策 — Amazonが廃止した理由と、日本で先に整理する4項目
excerpt: AI採用でバイアスが問題になるのは、AIが差別しようとするからではなく、過去の採用データの偏りをそのまま学習して増幅するからです。Amazonが採用AIを廃止した経緯とHireVueが顔分析をやめた前例をもとに、日本で運用する場合に先に整理すべき4項目と、バイアスへの具体的な対策を整理しました。
tag: AI仕事術
author: AIの鬼 編集部
date: 2026-07-19
updated: 2026-08-02
...
**結論から**：採用AIで効果が公表されているのは**書類の下処理**です。ソフトバンクはエントリーシート評価を年間680時間から約170時間へ、**約500時間削減**しました。一方で**評価そのものを任せた事例は撤回されています** — Amazonは女性を不利に扱う偏りが判明して開発中止、HireVueは顔分析を削除しました。日本で使う場合は、応募者情報を外部AIに入力する時点で個人情報保護法の整理が要ります。**下処理は任せ、合否の判断は人が持つ**のが現時点の線引きです。
...
### 効果が出ている使い方と、撤回された使い方

| | 何をさせたか | 結果 |
|---|---|---|
| **ソフトバンク** | エントリーシートの**評価補助**（5項目を評価） | 年間680時間 → **約170時間**（約500時間削減）。担当者の確認作業を75%程度軽減する見込み |
| **Amazon** | 履歴書を1〜5つ星で**評価** | 「women's」を含む履歴書を低く評価し、女子大2校の卒業生を格下げ。**開発中止** |
| **HireVue** | 動画面接で**顔の表情・動きを分析** | **2020年3月に削除**。顔分析は職務能力の予測として独立した科学的検証を受けていないと指摘されていた |

分かれ目は「下処理か、評価そのものか」です。
...
## バイアスへの対策 — 何をすれば減らせるのか

Amazonの事例が示すのは、**AIが差別しようと意図したのではなく、過去の人間の採用行動に含まれていた偏りをそのまま学習し、増幅した**ということです。したがって対策も「AIを賢くする」方向ではありません。

| # | 対策 | なぜ効くか |
|---|---|---|
| 1 | **AIの判定を最終決定にしない** | 人が再確認する工程を残せば、偏りが選考結果に直結しない |
| 2 | **学習させる過去データの偏りを先に確認する** | Amazonの原因は過去10年の履歴書。**入れるデータが偏っていれば結果も偏る** |
| 3 | **応募者を評価しない用途から始める** | 求人票やスカウト文の下書きなら、個人の評価に偏りが入り込まない |
| 4 | **属性と相関する情報を入力しない設計にする** | 性別・国籍を除いても、部活動名や出身校などから間接的に推定されることがある |
| 5 | **顔・表情・声の分析を選考の根拠にしない** | 職務能力の予測として独立した科学的検証を受けていない。HireVueは2020年3月に削除した |
| 6 | **判定結果を属性別に集計して点検する** | 「偏っていないつもり」では分からない。**通過率を属性別に数えると差が見える** |
| 7 | **AIを使っていることを応募者に説明する** | 説明できない選考は、後から問題になったとき擁護できない |

**6番が抜けやすい項目です。** 導入前にどれだけ設計しても、実際の通過率を集計しなければ偏りは見えません。定期的に数える工程を運用に入れてください。

**そして5番は、技術が進歩したから解決した話ではありません。** HireVueは自然言語処理の精度向上で顔分析が不要になったと説明していますが、専門家からは「顔分析は職務における能力や成功を予測するものとして独立に科学的検証を受けたことがない」「表情は文化・状況・障害によって異なるため選考の指標として信頼できない」という指摘がなされていました。
...
| 論点 | 内容 |
|---|---|
| **利用目的の明示** | 履歴書などは「採用選考のため」という目的の範囲内でしか使えない。取得時に目的を特定し本人に明示する |
| **第三者提供の同意** | 外部のAIサービスに応募者情報を入力する場合、原則として本人の同意が要る |
| **収集してはならない事項** | 人種・民族・社会的身分・門地・本籍・出生地など、社会的差別の原因となるおそれのある事項。**AIが意図せず収集しない設計**にする |
| **AI利用の説明** | AIを使っていること自体を応募者に説明し、必要な同意を得る |

## まとめ

- 効果が公表されているのは**書類の下処理**。ソフトバンクはエントリーシート評価を年間680時間→約170時間、**約500時間削減**した
- **評価そのものを任せた事例は撤回されている。** Amazonは「women's」を含む履歴書を低く評価する偏りが判明して開発中止、HireVueは顔分析を削除
- 分かれ目は「**下処理か、評価そのものか**」。合否の判断は人が持つ
- 顔分析は、職務における能力の予測として**独立した科学的検証を受けていない**と指摘されていた
- 日本で使うなら、**外部AIに応募者情報を入力する時点**で個人情報保護法の整理が要る。利用目的・第三者提供の同意・収集禁止事項・AI利用の説明の4点
- 入り口として軽いのは**求人票やスカウト文面の作成支援**。応募者個人を評価する場面ではない
- **過去の採用データに含まれる偏りは、意図せずAIに引き継がれる。** 導入後も定期的に点検する
- バイアス対策は7項目 — **判定を最終決定にしない／過去データの偏りを確認／応募者を評価しない用途から／属性と相関する情報を入れない／顔・表情・声を根拠にしない／通過率を属性別に集計する／AI利用を説明する**
- **抜けやすいのは「属性別に集計して点検する」。** 設計だけでは偏りは見えず、実際の通過率を数えて初めて差が出る

どの業務から始めるかは[中小企業が最初にAI化すべき業務の見つけ方](../ai-first-step-smb/)に、ツールの選び方は[AIツールの選び方](../ai-tool-selection/)に、社内で先に決めておくルールは[生成AIの社内ルールに何を書くか](../ai-internal-rules/)に整理しています。
