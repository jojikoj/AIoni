...
- 毎朝8時のブリーフィングは、**常時起動のPC1台・OS標準のPython・テキストファイル数本・cron**で動いている。追加ライセンスもAPI課金もクラウド利用料も無い
- **AIモデルは呼んでいない**。決まった条件でデータを絞って並べるだけなら、AIは要らない
- 実質の設計は「毎朝見たい情報は何か」を言葉で決め切ること。「危険かつS/Aランクの顧客」「期日が7日以内」のように条件を確定させる作業がすべてで、コードは後から付いてくる
- **この条件が決められない業務は、AIを使おうが使うまいが自動化できない**
- **3日間止まっていたが誰も気づかなかった。** 監査ログを見に行ったら、肝心の記録が残っていなかった
- ジョブを1本増やすたびに「**これが止まったらどうやって気づくか**」を一緒に決める。うちはそこを後回しにして3日ぶん取りこぼした
- 出ている事実は追えても、**読まれて行動に繋がったかは追えていない**。承認待ちが29→37件に増えているので、少なくとも一部は繋がっていない

止まった自動化を見張る仕組みは[止まったメディア更新を、見張り役が自動で叩き起こしていた話](../toe-media-watchdog/)に、動いているはずのジョブを数えた記録は[毎朝動いているはずのAI自動化41本を5.3日ぶん数えたら](../toe-cron-firing-audit/)にあります。
