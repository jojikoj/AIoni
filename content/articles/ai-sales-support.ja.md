...
**結論から**：営業支援AIで公表されている効果は、**記録と入力の手間を減らす**ところに集中しています。商談要点の整理と議事録作成の工数が約50%減、Salesforceへの登録件数が前年同期比で約20%増、という数字が出ています。一方で「**入力がゼロになる**」ではなく「**入力の手間が減る**」が実態に近く、専門用語や社内の略称が多い業界では確認・修正の工程が残ります。訪問先をAIに選ばせる使い方は、20年分のデータ蓄積があって初めて精度が出ています。
...
| 壁 | 中身 | どう受け止めるか |
|---|---|---|
| **提案を鵜呑みにできない** | 過去データに基づくため既存の取引パターンを踏襲しやすい。新規開拓や、履歴の薄い顧客・新商品では精度が落ちる | 提案は candidate であって決定ではない |
| **要約がずれる** | 専門用語や社内独自の略称が多い業界では、確認・修正の工程が残る | 「入力がゼロ」ではなく「入力の手間が減る」 |
| **学習する土台が薄い** | SFAの入力ルールや商談データが整っていないと効果が出るまで時間がかかる | 大塚商会は**20年分**の蓄積があって精度が出た |
| **コストと運用負荷** | 録音・要約系は月額課金が多く、人数が多いほど費用が増える。録音・解析には社内ルールの整備も要る | 導入して終わりではなく、**誰が運用ルールを管理するか**を決める |
...
- 公表されている効果は**記録と入力の手間を減らす**ところに集中している。商談要点の整理と議事録作成の工数が**約50%減**、SFAへの登録件数が**前年同期比 約20%増**
- ただし「入力がゼロになる」ではなく「**入力の手間が減る**」が実態。専門用語や社内の略称が多い業界では、確認・修正の工程が残る
- 訪問先をAIに選ばせる使い方は、**20年分のデータ蓄積**があって初めて精度が出ている。データが無い状態から高精度を期待するのは現実的ではない
- 提案は既存の取引パターンを踏襲しやすく、**新規開拓や履歴の薄い顧客には弱い**
- 始めるなら**記録業務の自動化から**。精度が多少ずれても営業活動そのものへの影響は限定的
- データが薄いなら、AIツールの導入より先に**商談記録を残す運用を整える**ほうが近道になることがある
- 録音・解析には社内ルールの整備が要る。**誰が運用ルールを管理するか**を決めておく
