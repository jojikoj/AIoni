...
| 増え方 | 誰が | どこから |
|---|---|---|
| 実測記事 | **人が書く** | 自社で実際に測った一次データ（捏造禁止。自動生成しない） |
| 旬ネタ記事 | 日次が自動で1本 | **その日に収集したニュース**（`tools/publish_daily.py`） |

旬ネタ記事は**ストックを持たない**。テーマ在庫から書くと「いつ書いても同じ記事」に
なり、それは既にAIが答えられる一般論で引用もクリックもされないため、当日の
`data/news.json` から `trend_news.pick()` で候補を取り、その素材（原文・自社解説）
だけで書く。コーナーはネタで決まる（AI検索・AEO系→AEO対策室／それ以外→AI解体新書）。

字数の下限は素材の量に連動する（8,000字以上→3,000字／3,000字以上→2,200字／
それ未満→1,500字）。**薄い長文より短い記事**にするため。

⚠️ **検査に落ちたものは公開しない。** 表・まとめ・「言えないこと」の節・内部リンク
2本以上（実在するslugのみ）・外部リンクは出典1本だけ・水増し表現ゼロ・そして
**当社の実測にも素材にも無い数字が1つでもあれば不合格**。落ちたら理由を伝えて1回だけ
書き直させ、それでも駄目なら次の候補へ。5件とも落ちた日は公開しない
（下書きは `content/_aeo_rejected/`、理由は日次ログ）。1日空くのは構わないが、
裏の取れない数字を載せるとこのサイトの前提が壊れる。
...
| 可視性チェッカー | トップと `/check/` に設置。会社名を入れるとAIがその企業をどう認識しているかをその場で測る（Vercel の `/api/diagnose`） |

## 案件獲得の導線（2026-08-01）

このサイトの目的はPVではなく、**AEO案件とAIエージェント案件の受注**。
検索流入を待つだけでは商談にならないので、こちらから動くための仕組みを持たせている。

```
記事末バナー（AI検索対策）──→ /check/ ──→ 実測 ──→ 結果画面の無料オファー
                                                        │ 社名と判定を引き継ぐ
                                                        ▼
                                                   /contact/?service=aeo&company=…
```

- **`/check/`** … AI検索対策の入口ページ。`aioni/templates/check.html`。
  記事末のAEOバナーは `/contact/` ではなくここへ送る。自社名を入れて
  「AIが自社を説明できていない画面」を自分で見た人だけが相談に進むので、
  フォームに届く時点で用件が固まっている。
- **測った社名と判定を `/contact/` へ引き継ぐ**。社名を打ち直させない。
  受け取る側も「どの会社が、どの判定で相談に来たか」が最初から分かる。
- **一括実測とレポート** … `tools/` の `batch_visibility.py` /
  `gen_ranking_article.py` / `gen_company_report.py`。
  企業リストをまとめて測り、記事（集計のみ公開）と個別レポート（1社ごとに送付）に分ける。
  詳細は `tools/README.md` と `data/companies/README.md`。
- **AIエージェントの実演** … `tools/demo_agent/`。提案の場でそのまま動かす。
