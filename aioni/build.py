...
class Builder:
    ...
    def build_lang(self, lang: str) -> None:
        ...
        for idx, n in enumerate(news):
            ...
            # ニュース個別ページは全件 noindex, follow にする（sitemap からも外す）。
            #
            # 経緯:
            #   ・08-15  全件 noindex（他社記事の要約が当社の顔になるのを止める）。
            #   ・08-16〜 Google の検索表示がサイト全体で 85/日 → 5/日。Bing は落ちず。
            #   ・08-25  「新着ニュースの鮮度流入を止めたのが原因」と見て、
            #            自社本文ありを index に戻した（sitemap 294 → 1,903 URL）。
            #   ・09-10  戻して16日（GSC の最新日）、表示は 2〜10/日 のまま戻っていない。
            #
            # 08-25 の見立て（原因は noindex）は、その後の実測で支持されなかった:
            #
            #   ・原因が noindex なら、index に戻した新着ニュースは数日で表示を
            #     取り戻すはず（08-25 の根拠自体が「新着は公開3日以内に稼ぐ」）。
            #     実際は16日間ゼロ近辺のまま。
            #   ・一度も noindex にしていない /articles/ も、28日表示 351 → 53
            #     （08-15 → 09-12）とニュースと同じ幅で落ちている。
            #   ・落ち始めは 08-14（表示41、前週の約半分）で、08-15 の公開より前。
            #
            # Google だけがサイト単位で評価を下げた、と見る方が数字に合う。
            # ドメイン取得（07-19）から2か月足らずで、他社記事の自動要約が個別ページの
            # 大半を占める状態は、スパムポリシーの Scaled content abuse
            # （大量生成コンテンツ）に当たりやすい。そうなら手当ては量産ページを
            # 検索対象から外すことで、鮮度流入を守ることではない。
            # 外して失うものも、いまはほぼ無い（ニュース個別 28日表示94・クリック0）。
            #
            # 答え合わせ: サイト単位の評価の見直しは数か月かかる。2026-12 に
            # /articles/ の28日表示が 351 まで戻っているかで判定する。戻っていなければ、
            # 次に疑うのは 07-19〜20 に一括投入した AI解体新書（128本）。
            #
