...
そこで元記事の本文を取得し、**1000字以上の日本語要約**を生成する。
...
TARGET_CHARS = 1000        # 生成する要約の最低文字数
...
要件:
- **必ず1000字以上**書くこと。短くまとめようとしないこと
- 5〜6段落に分け、段落間は空行で区切る
- 1段落目で「何が起きたか」を明確に述べる
- 2段落目以降で背景・経緯・技術的な内容・今後の見通しを説明する
- 製品名・モデル名・企業名は原綴りのまま残す（例: GPT-5, Claude, Hugging Face）
- 専門用語を使うときは、初出で一言そえて分かるようにする
...
def summarize(title: str, body: str) -> str:
    """本文から1000字以上の日本語要約を生成する。失敗時は空文字。"""
