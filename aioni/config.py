...
# トップページの <meta name="description">。
# SITE_DESCRIPTION は JSON-LD・OGP で使う正式な紹介文で197字あり、
# 検索結果では途中で切れて「…最新の研究・調査は中小」で終わっていた。
# Google が表示するのは概ね全角60〜70字なので、その中で
# 「誰が」「何を」「どう違うか」を言い切る。
HOME_META_DESCRIPTION = {
    "ja": "中小企業がAIを入れると実際どうなるか。株式会社TOEが自社で動かした"
          "処理件数・所要時間・失敗を、実ログの数字のまま公開しています。",
}
...
HERO_SUB = ("Claude・ChatGPT・Grok・Perplexity・AI Overviews 等を実際に測った数字。"
            "成果も失敗も、そのまま出す。")
# スマホ幅（720px以下）だけ差し替えるサブコピー。
# PC版はAI名を5つ並べるため縦3行になり、背景の鬼の顔と重なって読みにくい。
# 幅が狭いときは固有名を落とし、言いたいこと（実際に使った・隠さない）だけ残す。
HERO_SUB_SP = "AIを実際に使って、成果も失敗も、そのままのリアル情報をお届け。"
...
ARTICLE_CATEGORIES = [
    ...
    # AEO対策室（旧「AI検索観測所」の後継）
    #
    # 2026-08-13 の整理。「AI検索観測所」と「AI解体新書」は読者から見て
    # 中身の差が分からなかった（どちらも“解説が並んでいる”ように見える）。
    # コーナーとしては解体新書に一本化し、AI検索まわりの記事は
    # 「測った結果」も「どうすればいいか」も、この AEO対策室 に集約する。
    # 名前を「観測所」から「対策室」に変えたのは、読者が知りたいのが
    # 観測結果そのものではなく、自社が何をすればいいかだったため。
    {
        "id": "aeo",
        "tag": "AEO対策室",
        "name": "AEO対策室",
        "desc": "ChatGPT・Perplexity・Google AI Overviews に自社を見つけさせ、正しく説明させるための実務。手順・判断基準と、自社サイトで実際に測った数字を置く。効果が測れていないことは「測れていない」と書く。",
        "eyebrow": "対策",
    },
    {
        "id": "shippai",
...
# 記事個別URL（/articles/<slug>/）は slug を変えない限り動かないので、
# ここに載るのは一覧ページだけ。
REDIRECTS = {
...
# 保持する最大件数（アーカイブ蓄積の上限）
#
# ⚠️ NEWS_LIMIT は「個別ページが存在し続けるか」を直接決める値であって、
#    単なるデータ量の設定ではない。上限を超えた記事は news.json から
#    押し出され、次のビルドで /news/<slug>/ ごと消える。
#
#    2026-08-01 実測: 600 にしていたため、検索表示のあった237ページのうち
#    96ページ（表示の32%）が404だった。「kimi k3 使い方」で掲載順位2位・
#    表示22回のページも含まれる。Google がインデックスするのは公開の
#    2〜4週間後なので、1日30件収集×600件上限＝保持20日では
#    「検索結果に載った頃には消えている」状態が常に起き続ける。
#
#    3000 は 1日30件で約100日分。インデックス到達までの遅延に対して
#    十分な余裕があり、ビルド対象ページ数も現実的な範囲に収まる。
NEWS_LIMIT = 3000
PAPERS_LIMIT = 250

# 一覧ページ（/news/ とそのページ送り）に載せる件数。
# 個別ページは保持している全件を作るが、一覧まで全件に伸ばすと
# ページ送りが100ページを超え、クロールが一覧の断片に食われる。
# 一覧は「最近の流れを見る」ためのものなので直近分で足りる。
NEWS_LIST_LIMIT = 600

# 1回の収集で新しく翻訳する英語記事の上限。
#
# 上限を設けずに全未訳を回すと、2000件で1〜2時間かかる。途中で打ち切られると
# その回の成果ごと失われ、翌日も同じところからやり直しになる
# （2026-08-01 実測: 英語2272件中181件=7%で止まっていた）。
# 毎回この件数ずつ確実に前へ進める方が、結果として早く埋まる。
#
# items は新しい順なので、先頭から訳せば一覧に出る分から日本語になる。
TRANSLATE_PER_RUN = 80
